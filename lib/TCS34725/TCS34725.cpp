...
// Custom librarie to control the RGB sensor TCS34725 with led pin control in the constructor.
...
// Read the color
uint16_t RGB_Sensor::read_color(bool led_state) {
  ...
  // Store components of color in 16 bit variables
  uint16_t r = red_5bits & 0x1F;
  uint16_t g = green_6bits & 0x3F;
  uint16_t b = blue_5bits & 0x1F;

  // Merge components of color rgb565
  _last_color = ((r << 11) | (g << 5) | b);
...
uint16_t RGB_Sensor::get_last_color() {
  return _last_color;
}
